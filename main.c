...
void displayTime(){
  u16 affinity = (time_s + time_cs + time_m) > 0 ? ATTR0_AFFINE : 0;
  ...
  if (time_s_1s == 0 && time_cs_10s < 3) {
    shadowOAM[11].fill -= 8;  // AFFINE 2 y
  } else {
    shadowOAM[11].fill = 256;
  }

  if (time_s_1s == 0 && time_s_10s == 0 && time_cs_10s < 3) {
    shadowOAM[15].fill -= 8;  // AFFINE 3 y
  } else {
    shadowOAM[15].fill = 256;
  }

  if (time_s_1s == 0 && time_s_10s == 0 && time_m_1s == 0 && time_cs_10s < 3) {
...
void interruptHandler(void) {
  ...
  if (REG_IF & INT_TM1) {
    time_s--;
    time_cs=100;  // time_cs is inherently inaccurate, so this fixes some flipping glitches.
    if (time_s < 0) {
      time_s += 60;
      time_m--;
    }
    if (time_m < 0 || (time_s == 0 && time_cs == 0)) {
      time_m = 0;
      time_s = 0;
      time_cs = 0;
      pal0_color = RED;
      ...
      pal0_color = COLOR(percent_complete * 32 / 100,  20, 0);
    }
    REG_IF = INT_TM1;
  }
